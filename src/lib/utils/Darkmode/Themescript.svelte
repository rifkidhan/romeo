<script lang="ts">
	...
	export let attrs: any; //eslint-disable-line @typescript-eslint/no-explicit-any
	...
	$: themeScript = `<${'script'}>${
		forcedTheme
			...
			? `!function(){try {${optimization}var e=localStorage.getItem('${storageKey}');${
			  ...
			  }if("system"===e||(!e&&${defaultSystem})){var t="${'(prefers-color-scheme: dark)'}",m=window.matchMedia(t);if(m.media!==t||m.matches){${updateDOM(
					'dark'
